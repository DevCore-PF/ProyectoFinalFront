...
const CourseDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const checkCourseAccess = async () => {
      ...
      try {
        ...
        const isOwner = user?.professorProfile && 
                       typeof user.professorProfile === "object" &&
                       course.professor?.id === user.professorProfile.id;
        ...
        const purchasedCourses = await purchasedCoursesService.getMyPurchasedCourses(token);
    ...
    const checkCourseCompletionAndFeedback = async () => {
      if (course && course.lessons && !progressLoading && user && token) {
        ...
        if (isCourseCompleted(totalLessons)) {
          ...
          try {
            const hasFeedback = await courseFeedbackService.hasUserFeedback(courseId as string, token);
  ...
  }, [course, totalCompleted, progressLoading, isCourseCompleted, courseId, user, token]);
  ...
  const handleLessonCompletion = async (lessonId: string) => {
    try {
      ...
      setProgressError('Error al marcar lección como completada. Inténtalo de nuevo.');
  ...
  return (
    <div className="min-h-screen  p-8">
          ...
          {progressError && (
            <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="text-red-400 font-medium">Error de progreso</p>
                  <p className="text-red-300 text-sm">{progressError}</p>
                </div>
                <button
                      ...
                      <div className="p-4 bg-slate-900/20 border-t border-slate-600/30">
                        ...
                        {!hasAccessToCourse ? (
                                ...
                                <button
                                  ...
                                  className={`px-6 py-2 rounded-lg font-medium transition-colors ${
                                    isInCart
                                      ...
                                      : "bg-button hover:bg-button/80 text-white"
                                  }`}
                                >
                          ...
                          <>
                            ...
                            {/* Contenido Adicional */}
                            {lesson.aditionalData && lesson.aditionalData.length > 0 && (
                              <div className="mb-4">
                                <h4 className="text-slate-300 font-medium mb-3 flex items-center gap-2">
                                  ...
                                  Contenido Adicional ({lesson.aditionalData.length})
                                </h4>
                                <div className="space-y-2">
                                  {lesson.aditionalData.map((urlData: string, urlIndex: number) => {
                                    ...
                                    const displayName = getDomainDisplay(urlData);
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <div className="flex-1">
                                          <span className="text-slate-300 group-hover:text-white transition-colors block">
                                            {displayName}
                                          </span>
                        ...
                        {hasAccessToCourse && !(user?.professorProfile && 
                          ...
                          <button
                            ...
                            disabled={isCompleted || completingLessons.has(lesson.id)}
      ...
      {course && course.lessons && isCourseCompleted(course.lessons.length) && !hasProvidedFeedback && !showingFeedback && (
        <div className="mt-8 p-6 bg-gradient-to-br from-slate-800/30 to-slate-700/30 border border-slate-600/30 rounded-2xl">
          ...
          <div className="text-center">
            <button
              ...
              className="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-8 py-3 rounded-xl hover:from-blue-600 hover:to-purple-700 transition-all duration-300 shadow-lg hover:shadow-xl"
            >
      ...
      {course && course.lessons && isCourseCompleted(course.lessons.length) && hasProvidedFeedback && (
        <div className="mt-8 p-6 bg-gradient-to-br from-green-800/20 to-emerald-700/20 border border-green-600/30 rounded-2xl">
          <div className="text-center">
            ...
            <p className="text-green-300/80">
              Tu opinión es muy valiosa para nosotros y nos ayuda a mejorar nuestros cursos.
            </p>
          </div>
        </div>
      )}
