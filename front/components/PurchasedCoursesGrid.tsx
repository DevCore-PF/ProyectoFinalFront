    ...
    <div className="bg-[#1e1e2e] border border-slate-700/50 rounded-xl overflow-hidden hover:border-slate-600/50 transition-all duration-300 hover:shadow-lg hover:shadow-[#3f4273]/20">
      ...
      <div className="relative p-6 pb-4">
        ...
        <div className="space-y-2">
          <h3 className="text-lg font-bold text-font-light mb-2 line-clamp-2">
            {course.title}
          </h3>
                  ...
                  >
                    ...
                    <span
                      className={`line-clamp-1 ${
                        lesson.completed ? "text-gray-300" : "text-gray-500"
                      }`}
                    >
                      {lesson.title}
                    </span>
...
export const PurchasedCoursesGrid: React.FC<PurchasedCoursesGridProps> = ({
  className = "",
}) => {
    ...
    return (
      <div className={`space-y-6 ${className}`}>
        <div className="text-center py-16 text-slate-400 bg-slate-900/30 rounded-xl border border-slate-700/20">
          ...
          <Link
            href="/courses"
            className="inline-block bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 px-6 py-3 rounded-lg text-font-light font-medium transition-all duration-300"
          >
            Explorar cursos
          </Link>
        </div>
        ...
        <div className="text-sm text-gray-400">
          {
            purchasedCourses.filter(
              (course) => course.completed || course.progress >= 100
            ).length
          }{" "}
          completados
        </div>
